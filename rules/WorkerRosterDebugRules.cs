...
internal static class WorkerRosterDebugRules
{
    internal static string GetFileName(string snapshotName)
    {
        return $"worker-roster-{Sanitize(snapshotName)}.json";
    }

    internal static string BuildJson(IReadOnlyList<WorkerData> entries, IReadOnlyDictionary<int, GeneratedWorkerAppearance> generatedAppearances)
    {
        return BuildJson(entries, generatedAppearances, WorkerManager.GetWorkerList());
    }

    internal static string BuildJson(IReadOnlyList<WorkerData> entries, IReadOnlyDictionary<int, GeneratedWorkerAppearance> generatedAppearances, IReadOnlyList<Worker> runtimeWorkers)
    {
        ...
        {
            WorkerData worker = entries[index];
            bool isGenerated = generatedAppearances.ContainsKey(index);
            builder.Append("  {");
            builder.Append("\"workerIndex\":").Append(index);
            builder.Append(",\"name\":\"").Append(Escape(worker.GetName())).Append('"');
            builder.Append(",\"isGenerated\":").Append(isGenerated ? "true" : "false");
            builder.Append(",\"shopLevelRequired\":").Append(worker.shopLevelRequired);
            builder.Append(",\"costPerDay\":").Append(Format(worker.costPerDay));
            builder.Append(",\"hiringCost\":").Append(Format(worker.hiringCost));
            builder.Append(",\"restockSpeed\":").Append(Format(worker.restockSpeed));
            builder.Append(",\"checkoutSpeed\":").Append(Format(worker.checkoutSpeed));
            builder.Append(",\"walkSpeedMultiplier\":").Append(Format(worker.walkSpeedMultiplier));
            builder.Append(",\"prologueShow\":").Append(worker.prologueShow ? "true" : "false");
            builder.Append(",\"hasIcon\":").Append(worker.icon != null ? "true" : "false");
            if (worker.icon?.texture != null)
            {
                Texture texture = worker.icon.texture;
                builder.Append(",\"iconTexture\":{\"width\":").Append(texture.width);
                builder.Append(",\"height\":").Append(texture.height).Append('}');
            }

            Worker? runtimeWorker = index < runtimeWorkers.Count ? runtimeWorkers[index] : null;
            if (runtimeWorker != null)
            {
                builder.Append(",\"runtime\":{");
                builder.Append("\"isFemale\":").Append(runtimeWorker.m_IsFemale ? "true" : "false");
                builder.Append(",\"isActive\":").Append(runtimeWorker.m_IsActive ? "true" : "false");
                AppendCharacterCustomization(builder, runtimeWorker.m_CharacterCustom);
                builder.Append('}');
            }

            if (isGenerated && generatedAppearances.TryGetValue(index, out GeneratedWorkerAppearance? appearance))
            {
                builder.Append(",\"generatedAppearance\":{");
                builder.Append("\"isFemale\":").Append(appearance.IsFemale ? "true" : "false");
                builder.Append(",\"characterModelIndex\":").Append(appearance.CharacterModelIndex);
                builder.Append('}');
            }
